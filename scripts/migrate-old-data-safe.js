...
function mapCategory(tags, mapping) {
  if (!tags || tags.length === 0) return mapping['Numérique'] || 'GENERAL';
  ...
  return mapping['Numérique'] || 'GENERAL';
}
...
async function cleanDatabase() {
  ...
  if (isTestMode || isDryRun) {
    console.log('⚠️  Mode test/dry-run - Pas de nettoyage effectué');
    return;
  }
  
  try {
    // Supprimer dans l'ordre pour respecter les contraintes de clés étrangères
    console.log('  - Suppression des votes...');
    await prisma.vote.deleteMany({});
    
    console.log('  - Suppression des commentaires de forum...');
    await prisma.forumComment.deleteMany({});
    
    console.log('  - Suppression des posts de forum...');
    await prisma.forumPost.deleteMany({});
    
    console.log('  - Suppression des conseils...');
    await prisma.tip.deleteMany({});
    
    console.log('  - Suppression des articles...');
    await prisma.article.deleteMany({});
    
    console.log('  - Suppression des initiatives...');
    await prisma.initiative.deleteMany({});
    
    console.log('  - Suppression des demandes de rôles...');
    await prisma.roleRequest.deleteMany({});
    
    console.log('  - Suppression des tâches utilisateur...');
...
async function createDefaultAdmin() {
  ...
  if (isTestMode || isDryRun) {
    console.log('⚠️  Mode test/dry-run - Pas de création d\'admin');
    return null;
  }
  ...
  try {
    const adminUser = await prisma.user.create({
      data: {
        ...
        username: 'admin',
        password: '$2a$10$default.admin.password.hash', // À changer après migration
        role: 'ADMIN',
        emailConfirmed: true,
        name: 'Administrateur Polux'
      }
    });
...
async function main() {
  try {
    if (isTestMode) {
      ...
      console.log('🔍 Mode dry-run activé - Simulation de la migration');
    } else {
      console.log('⚠️  ATTENTION: Ce script va nettoyer complètement la base de données avant la migration');
    ...
    if (isTestMode) {
      ...
      console.log('Pour lancer la vraie migration, exécutez: node scripts/migrate-old-data-safe.js');
    } else if (isDryRun) {
      console.log('🎉 Simulation de migration terminée avec succès !');
      console.log('Pour lancer la vraie migration, exécutez: node scripts/migrate-old-data-safe.js');
    } else {
      console.log('🎉 Migration terminée avec succès !');
      console.log('⚠️  N\'oubliez pas de changer le mot de passe de l\'utilisateur admin (admin@polux.fr)');
    }
  } catch (error) {
